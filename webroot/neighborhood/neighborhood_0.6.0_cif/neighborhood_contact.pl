...
#------------------------------ CONTACT -----------------------------------------
my $contact62_exec = "/var/www/html/cmm3/webroot/neighborhood/ccp4-6.4.0/bin/contact";
...
my $NEIGHBORHOOD     ="/var/www/html/cmm3/webroot/neighborhood/neighborhood_0.6.0_cif";
$ENV{"NEIGHBORHOOD"} = $NEIGHBORHOOD;
my $exec_dir         ="$NEIGHBORHOOD/src";
my $exec_prog        ="$exec_dir/neighborhood";
my $temporary_dir    ="/var/www/html/cmm3/webroot/neighborhood_temp/".$ARGV[0];
print "aaa\n";
my $return_code      = calculate_contact_file($temporary_dir,$pdbfile,$contactfile);
if ($return_code) {
  $contactfile = "NULL" 
}
my $exec_line	     ="$exec_prog $temporary_dir $pdbfile XXXX.cluster $contactfile NULL";
...
sub calculate_contact_file{
  ...
  my $pdbfile="$temporary_dir/model\.pdb";
  ...
  while (<ZIPFILE>) {
    if (/^EXPDTA.*NMR/) {
      $retcode=100;	# return error in case of NMR, since no contact need to be calculated
    } elsif (/^MODEL/) {
      $count_num_models++;
    } elsif (/^CRYST1 (....\.\d\d\d) (....\.\d\d\d) (....\.\d\d\d) (...\.\d\d) (...\.\d\d) (...\.\d\d)/) {
      $cryst1=1 unless $1==1 and $2==1 and $3==1 and $4==90 and $5==90 and $6==90;
    }
  }
  close ZIPFILE;
  $retcode=101 unless $cryst1;	# return error in case of no CRYST1 card encountered, since no contact need to be calculated
  ...
  $retcode = run_and_convert($pdbfile,$logfile,$contactfile,62) unless $retcode;
