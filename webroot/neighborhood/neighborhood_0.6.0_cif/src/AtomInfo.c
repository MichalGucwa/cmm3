...
//   }
  ...
  if(e[0]=='H' && I->ResidueInfo->type<=RESIDUE_DNARNA) {
  ...
  if(I->protons==0) {
    ...
    if(!elementFound && e[0]=='H') {     // For X in ASX and GLX
      elementFound=1;
  ...
  if(I->protons==AN_C || I->protons==AN_O || I->protons==AN_N || I->protons==AN_S || I->protons==AN_P || I->protons==AN_Se || I->protons==AN_H) {
    I->isMetal=0;
...
void AtomInfoPrint(ObjAtomInfo *I) {
  ...
  printf("%d: res.chain %c; alt %c; res.resn %s; res.resi %s; realname %s; elem %s; vdw %f\n",
       I->id, resid->chain, I->alt, resid->resn, resid->resi, I->realname, I->elem, I->vdw);
}
  ...
  if (resFound==-1) {
    ...
    if(atomFound==-1){
      ...
      if(atomFound==-1){
        ...
        if(atomFound==-1) {
          sprintf(msg,"Failed to locate corresponding atom %d:%s:%c for SURFRACE file.\n", atmi->id, atmi->realname, atmi->alt);
          PFatalError(msg);
        }
      }
    }
...
int AtomInfoLocateIndex4contact(int* list_atomindex, ObjResidueInfo *resi, ObjAtomInfo *atmi, ObjEntityMolecule *I, int prev_atom_index) {
  int i, j, atomnamelen, resFound, atomFound, resException;
...
//    if(!strncmp(atmi->name, ai->realname, 4)) {
// This will fail because CCP4 output calcium atom as -CA- while PDB output calcium atom as CA--
