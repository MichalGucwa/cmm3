...
my $NEIGHBORHOOD       ="/home/sg/neighborhood/neighborhood_0.6.0_src";
my $MYPDB_NEW          ="/med/zenobi_data/MYPDB_NEW";
my $PSQL_DIR           ="/usr/bin";
my $NEIGHBORHOODTEMP   =".";
my $NEIGHBORHOOD_DEBUG =0;
...
my $AU_contact_dir     ="$MYPDB_NEW/AU2_contact";                # file name pattern: pdb100d.contact
...
my $database_name      ="neighborhood11";
...
if (exists $ARGV[0]) {
  if (($ARGV[0] eq "-l") or ($ARGV[0] eq "-f")) {
    ...
    if ($use_subset) {
      $abinitio=1;
      $abinitio=0 if ((exists $ARGV[2]) and ($ARGV[2] eq "0"));
      if($file_restrict_pdbid_set_path =~ /\/([^\/]+)$/) {
        $file_restrict_pdbid_set_name=$1;
      } else {
        $file_restrict_pdbid_set_name=$file_restrict_pdbid_set_path;
      }
      $database_name="neighborhood11_$file_restrict_pdbid_set_name";
      $temporary_dir="$NEIGHBORHOODTEMP/temp_for_$file_restrict_pdbid_set_name";
    }
  } elsif ($ARGV[0] eq "1") {
    $abinitio=1;
  } elsif ($ARGV[0] eq "0") {
...
CREATE TABLE geneontology(
    ...
    go_id               integer,
    evidence            char(1) NOT NULL,       -- EXP, IMP, IC, IGI, IPI, ISS, IDA, IEP, IEA, TAS, NAS, NR, ND or RCA.
    ref_db_name         char(20),
    ref_db_id           integer,
    ...
    if (/^\s*!.*$/) {
      ...
      if($#fields == 16) {
        ...
        if($fields[6] eq "EXP" or $fields[6] eq "IMP" or $fields[6] eq "IC"  or $fields[6] eq "IGI" or $fields[6] eq "IPI" or
           ...
           $fields[6] eq "NAS" or $fields[6] eq "NR"  or $fields[6] eq "ND"  or $fields[6] eq "RCA") {
          $evidence=$fields[6];
        } else {
...
sub generate_cluster_file {
  ...
  format_six_digit($start_pdbfileid, 1);
  if ($flag >= 2) {
...
sub process_sql_file {
  my $filename=shift;
  my $sqlcommand="$psql_prog -p$database_port $database_name < $filename\n";
  print $sqlcommand;
...
sub process_sql_command {
  my $command=shift;
  my $sqlcommand="$psql_prog -p$database_port $database_name << HEPING\n$command\nHEPING\n";
  print $sqlcommand;
...
if ($abinitio) {
  ...
  my $ip_address="";
  my $mac_address="";
  open IFCONFIG, 'ifconfig |' or die 'Cannot execute ifconfig\n';
  while (<IFCONFIG>) {
  ...
  close IFCONFIG;
  process_sql_command("INSERT INTO neighborhood (purpose,version,host,ip,port,mac_address,database_name) values ('general','0.6.0','$database_host','$ip_address',$database_port,'$mac_address','$database_name');");
...
process_sql_file($sql_update_begin) if ($abinitio == 0);
...
# } elseif {$flagflag == 0} { continue }
  ...
  if($single_mode) {
    ...
    print CLUSTEROUT "PDBFILE      0";
    close CLUSTEROUT;
  } else {
    if($use_subset) {
      next if (not exists $pdbid_dict{$lower_pdbid});
    }
    ...
    $AU_contact_file = "pdb$lower_pdbid.contact";
...
# 3. $AU_contact_file -- copy from $AU_contact_dir/pdb1abc.contact
# 4. $BU_header_file  -- copy from $BU_header_dir/pdb1abc.header
...
# STEP 3: generating $AU_contact_file
    $filename    = "$AU_contact_dir/$AU_contact_file";
    $outfilename = "$temporary_dir/$AU_contact_file";
    if (-e $filename and -f $filename) {
      system("cp $filename $outfilename") == 0 or die "Could not copy file $filename\n";
    } else {
      $AU_contact_file = "NULL";
    }
    print "CONTACT... ";

# STEP 4: generating $BU_header_file
    $filename    = "$BU_header_dir/$BU_header_file";
    $outfilename = "$temporary_dir/$BU_header_file";
    if (-e $filename and -f $filename) {
      system("cp $filename $outfilename") == 0 or die "Could not copy file $filename\n";
    } else {
      $BU_header_file = "NULL";
...
# argv[4]: $AU_contact_file     -- text file that specify the contact info     (in '|' delimited format) (calculated by CCP4 contact program)
# argv[5]: $BU_header_file      -- header that defines all biological unit related info (with cavity calculated by surflex-dock, and surface area calculated by surface-racer)
  my $exec_line = "$exec_prog $temporary_dir $pdbfile $cluster_file $AU_contact_file $BU_header_file $MYPDB_NEW";
  print "$exec_line\n";
