...
void PDBheaderAssignCompnd(ObjEntityMolecule *I, char *line, int continuation, int isEoS) {
  ...
  if(continuation) {
    ...
    if(!strncmp(p,"MOL_ID: 1;",10)) {
      ...
      sprintf(msg,"Unexpected first line (%s).\n",I->pdbid);
      compound_error=2;
    }
  }
  if        (I->compnd_wait) {                 compndFlag=10; keyLength=0;      // string
    ...
    sprintf(msg,"Unexpected pattern %s (%s).\n",p,I->pdbid);
    compound_error=3;
  }
  if(compndFlag) {
    ...
    if(isOpenLine) {
      ...
      if(tempFieldLength<TemporaryFieldLen-FileLineLength) {
        ...
        sprintf(msg,"Field pattern (%s) is too long (%s).\n",I->temp_field,I->pdbid);
        compound_error=4;
      }
      if(isCloseLine) {
    ...
    if(compndFlagClose) {
      ...
      if(I->temp_field[tempFieldLength-1]!=';') {
        sprintf(msg,"Field pattern (%s) does not end with semicolon (%s).\n",I->temp_field,I->pdbid);
        compound_error=5;
      }
      ...
      if(compndFlagClose==1) {
        ...
        if(comi->index==-1) {
          sprintf(msg,"Field pattern (%s) being read without MOL_ID field (%s).\n",p,I->pdbid);
          compound_error=6;
        }
...
void PDBheaderAssignSource(ObjEntityMolecule *I, char *line, int continuation, int isEoS) {
  ...
  if(continuation) {
    if (continuation!=I->source_done+1) {
      sprintf(msg,"Unexpected SOURCE continuation mark encountered, expecting %d, got %d (%s).\n", I->source_done+1, continuation, I->pdbid);
      source_error=1;
    }
  ...
  } else if (!strncmp(p,"FRAGMENT"           , 8)) { sourceFlag=6;  keyLength=8;
  } else if (!strncmp(p,"STRAIN"             , 6)) { sourceFlag=7;  keyLength=6;
  } else if (!strncmp(p,"VARIANT"            , 7)) { sourceFlag=8;  keyLength=7;
  } else if (!strncmp(p,"CELL_LINE"          , 9)) { sourceFlag=9;  keyLength=9;
  } else if (!strncmp(p,"CELLULAR_LOCATION"  ,17)) { sourceFlag=10; keyLength=17;
  } else if (!strncmp(p,"CELL"               , 4)) { sourceFlag=11; keyLength=4;
  } else if (!strncmp(p,"ATCC"               , 4)) { sourceFlag=12; keyLength=4;
  } else if (!strncmp(p,"COLLECTION"         ,10)) { sourceFlag=12; keyLength=10;
  } else if (!strncmp(p,"ORGAN"              , 5)) { sourceFlag=13; keyLength=5;
  } else if (!strncmp(p,"TISSUE"             , 6)) { sourceFlag=14; keyLength=6;
  } else if (!strncmp(p,"ORGANELLE"          , 9)) { sourceFlag=15; keyLength=9;
  } else if (!strncmp(p,"SECRETION"          , 9)) { sourceFlag=16; keyLength=9;
  } else if (!strncmp(p,"PLASMID"            , 7)) { sourceFlag=17; keyLength=7;
  } else if (!strncmp(p,"GENE"               , 4)) { sourceFlag=18; keyLength=4;
  } else if (!strncmp(p,"OTHER_DETAILS"      ,13)) { sourceFlag=19; keyLength=13;
  } else if (!strncmp(p,"EXPRESSION_SYSTEM"  ,17)) { sourceFlag=20; keyLength=17;
  } else if (I->source_wait) {                       sourceFlag=21; keyLength=0;
  ...
  if(sourceFlag<=4) {
    ...
    if(sourceFlag==1) {
      ...
      if(!componentFound) {
        sprintf(msg,"Cannot locate corresponding component number (%d) for SOURCE card.\n",componentnum,I->pdbid);
        source_error=4;
      }
    } else {
      if(I->temp_index<0 || I->temp_index>=I->numComponent) {
        sprintf(msg,"Index to component (%d) is out of range.\n",I->temp_index,I->pdbid);
        source_error=5;
      }
