...
class DefMap(dict):
   ...
   def __init__(self, in_def_path, typed):
      def_path = os.path.abspath(in_def_path)
   ...
   def get_viewers(self):
      if self.has_key("EXPORT_TO_QTRVIEW") and self.has_key("RUN_COOT") and self.has_key("RUN_CCP4MG"):
...
def from_command_line(log_file_in):
   ...
   generator = os.path.join(ccp4path, "share", "smartie", "qtrgeneric.py")
   ...
   try:
      ...
      rep_inp_data = list()
      rep_inp_data.append("PYTHON " + os.path.realpath(sys.executable))
