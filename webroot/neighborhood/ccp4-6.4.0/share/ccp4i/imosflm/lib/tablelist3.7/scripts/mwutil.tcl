# $Id: mwutil.tcl,v 1.8 2012/05/21 13:23:39 ccb Exp $
#==============================================================================
...
namespace eval mwutil {
    ...
    namespace export	wrongNumArgs getAncestorByClass convEventFields \
			defineKeyNav generateEvent configure \
...
#
# Generates the given event for the mega-widget of the specified class
# containing the widget w if that mega-widget is not the only widget receiving
# the focus during keyboard traversal within its top-level widget.
#------------------------------------------------------------------------------
proc mwutil::generateEvent {w class event} {
    set win [getAncestorByClass $w $class]
    if {[string compare [tk_focusNext $win] $win] != 0} {
	focus $win				;# necessary on Windows
...
proc mwutil::getScrollInfo argList {
	...
	if {[string first $what "units"] == 0 ||
	    [string first $what "pages"] == 0} {
	    return [list scroll $number $what]
	} else {
