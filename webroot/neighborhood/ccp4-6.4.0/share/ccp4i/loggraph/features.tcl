...
proc bltFindElement { graph x y } {
    ...
    set x [lindex [lindex [$graph element configure $name -xdata] 4] $index]
    set y [lindex [lindex [$graph element configure $name -ydata] 4] $index]
 # [$graph tag create text $points -text " $name \[$index\] " -anchor s \
    set tagId($graph,$name,$index) \
      [$graph tag create text $points -text " $name \[$x\] \[$y\]" -anchor s \
...
proc bltScan { graph x y } {
    global pos
    if { ![info exist pos(last,x)] } {set pos(last,x) 0}
    if { ![info exist pos(last,y)] } {set pos(last,y) 0}
