...
proc RGBInit {} {
    ...
    if { ![catch {set f [open /usr/lib/X11/rgb.txt r]} ] } {
	set colorList [split [read $f] \n]
	foreach i $colorList {
	    set parts [split $i \t]
...
proc RGBInitControls { name option color } {
	...
	$frame.scale_$i set [expr [set rgb-${name}${option}($i)]]
    }
}
...
proc FontControl { name option value } {
    ...
    table column $frame configure 1 -resize none
    $frame.entry insert 0 $value
...
proc ConfigureGeneral {} {
    ...
    table column $table($name) configure { 1 5 } -padx 5
    table row $table($name) configure all -pady 2
    table column $table($name) configure { 0 4 9 } -width 0.25i
    table row $table($name) configure { 0 $row-1 } -height 0.125i
    table row $table($name) configure { $row } -height {}
}
...
proc ConfigureAxis {} {
    ...
    table column $table($name) configure 1 -padx 5
    table row $table($name) configure all -pady 2
    table column $table($name) configure { 0 4 } -width 0.25i
    table row $table($name) configure { 0 $row-1 } -height 0.125i
    table row $table($name) configure { $row } -height {}
}
...
proc ConfigureElement {} {
    ...
    foreach i $elements {
	regsub -all " " $i "" wnm
	radiobutton $frame.rbutton_$wnm -variable element -value $i -text $i \
	    -command "SetElemOptions"
	table $frame $frame.rbutton_$wnm $count/2,$count%2 -anchor w
	incr count
    }
