...
#------------------------------------------------------------------------
  proc phaser_TNCS_setup { typedefVar arrayname } {
#------------------------------------------------------------------------
...
#------------------------------------------------------------------------
proc phaser_TNCS_run { arrayname } {
#------------------------------------------------------------------------

  upvar #0 $arrayname array

# The output file array is set up in phaser_TNCS.script
...
#------------------------------------------------------------------------
 proc phaser_TNCS_component { arrayname counter_component } {
#------------------------------------------------------------------------
...
#------------------------------------------------------------------------
 proc phaser_TNCS_set_test_space_group { arrayname } {
#------------------------------------------------------------------------
...
# update the list of spacegroups varlabel
  phaser_TNCS_updatevarlabel $arrayname FILE_SPACEGROUP
  if { $laue_no <= 0 } {
...
#------------------------------------------------------------------------
 proc phaser_TNCS_updatevarlabel { arrayname FILE_SPACEGROUP } {
#------------------------------------------------------------------------
...
#
#------------------------------------------------------------------------
  proc phaser_TNCS_set_reso {arrayname} {
#------------------------------------------------------------------------
  upvar #0 $arrayname array
...
#------------------------------------------------------------------------
  proc phaser_TNCS_maca { arrayname counter_s } {
#------------------------------------------------------------------------
  global configure
  upvar #0 $arrayname array
  CreateLine line \
...
#------------------------------------------------------------------------
  proc phaser_TNCS_mact { arrayname counter_s } {
#------------------------------------------------------------------------
  global configure
  upvar #0 $arrayname array
  CreateLine line \
...
#------------------------------------------------------------------------
  proc phaser_TNCS_task_window {arrayname} {
#------------------------------------------------------------------------
  ...
  if { [CreateTaskWindow $arrayname \
        "Phaser Translational NCS/Twin Detection"\
        "Phaser" \
...
#   whether rotation, translation or refinement/rescoring searches
  ...
  CreateInputFileLine line \
      ...
      HKLIN_MAIN DIR_HKLIN_MAIN \
        ...
        -command "phaser_TNCS_set_test_space_group $arrayname" \
        -command "phaser_TNCS_set_reso $arrayname"
...
# The "Defie composition" folder
  ...
  CreateExtendingFrame N_COMPONENT phaser_TNCS_component \
...
# The "Expert parameters" folder
  ...
  CreateLine line \
    ...
    OpenSubFrame frame \
     ...
     CreateExtendingFrame N_MACA phaser_TNCS_maca \
    ...
    CreateLine line \
      message "Refinement macrocyles" \
      widget TOG_MACT -toggleon \
      label "Translational NCS refinement protocol" \
      widget MACT_PROTOCOL

    OpenSubFrame frame \
      -toggle_display MACT_PROTOCOL open CUSTOM hide
     CreateExtendingFrame N_MACT phaser_TNCS_mact \
        "Customize the anisotropic refinement" \
        "Add another anisotropic macrocycle" \
        [list MACT_ROT MACT_TRA MACT_VAR MACT_NCYC ]
    CloseSubFrame
