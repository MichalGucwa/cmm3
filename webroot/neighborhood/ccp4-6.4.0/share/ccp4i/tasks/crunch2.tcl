...
proc crystal_proc { arrayname counter } {
	...
	CreateLine line \
	    label "Native" \
	    message "Crystal is a native crystal : No heavy atoms, not anomalous" \
	    widget CRYSTAL_NATIVE \
	    label "                                                       " \
	    label "                                                       "
    }
...
proc rebuild_varmenus { arrayname } {
    ...
    # check if the refmac version has the SAD function:
    set refmac_sad 0
    ...
    # check if the refmac version has the SIRAS function:
    set refmac_siras 0
    if { [file exists [FindExecutable refmac5]] } {
	set refmac_command "refmac5 -i"
	set refmac_script "\nEND\n"
	...
	# By default, use fast phasing in bp3 for a MAD experiment
	...
	if { $program_name == "refmac" } {
        ...
        if { ( ($program_name == "pirate") || ($program_name == "resolvedm") ||
            ...
            set len [llength $menulist_phase_columns($step)]
            for { set i [expr $len -1 ] } { $i >= 0 } { set i [expr $i - 1] } {
                set phase_column_i [lindex $menulist_phase_columns($step) $i]
                if { ![regexp {([0-9]*)_(.*)} $phase_column_i full regexp_step regexp_program] } {
                    puts "rebuild_varmenu::Could not match step and program for phase_columns=($phase_columns)."
                    puts "This shouldn't ever happen - please email crank@chem.leidenuniv.nl the details."
                    exit
                }
                if { $regexp_program == "SOLOMON" } {
                    set found_solomon_before_denmod 1
                    set array(SOLOMON_DENMOD,$regexp_step) 0
                    break
                }
                if { $regexp_program == "SHELXE" } {
                    set found_shelxe_before_denmod 1
                    set array(SHELXE_DENMOD,$regexp_step) 0
                    break
                }
            }
...
proc experiment_proc { arrayname counter } {
    ...
    # It would seem to make sense to rebuild the varmenus here, but
    # we actually want to rebuild them before we add new menu items.
    if { [info exists building_pipeline] } {
 	if { $building_pipeline == 0 } {
 	    rebuild_varmenus $arrayname
 	}
    } else {
 	rebuild_varmenus $arrayname
...
proc clear_pipeline { arrayname } {
	...
	foreach subframe $all_subframes {
	    ...
	    trace vdelete array(PREMADE_PIPELINE) w "update_main_scroll $subframe"
	}
...
proc add_premade_pipeline { arrayname } {
    ...
    set premade_pipeline $array(PREMADE_PIPELINE)
    ...
    if { [string compare $premade_pipeline "CRUNCH2/BP3/SOLOMON/ARPWARP"] == 0 } {
	...
	if { ( ( ([ string compare $start "Substructure detection"] == 0) ||
		 ([ string compare $start "Substructure phasing"]   == 0) ) && 
	       !$done ) } {
	    set array(EXPERIMENTAL_PROGRAM) "BP3"
	...
	if { ( ( ([ string compare $start "Substructure detection"] == 0) ||
		 ...
		 ([ string compare $start "Density modification"]   == 0) ) &&
	       !$done ) } {
 	    set array(EXPERIMENTAL_PROGRAM) "SOLOMON"
	...
	if { [file exists [FindExecutable arp_warp]] && !$done } {
	    set array(EXPERIMENTAL_PROGRAM) "ARPWARP"
    ...
    } elseif { [string compare $premade_pipeline "SHELXCDE/BP3/SOLOMON/ARPWARP"] == 0 } {
	set array(EXPERIMENTAL_PROGRAM) "PREP"
	update_toggleframe0 experiment_proc 0 $arrayname N_PROGRAMS 1 0
	...
	if { ( ( ([ string compare $start "Substructure detection"] == 0) ||
		 ([ string compare $start "Substructure phasing"] == 0) ) && 
	       !$done) } {
 	    set array(EXPERIMENTAL_PROGRAM) "SHELXE"
	    update_toggleframe0 experiment_proc 0 $arrayname N_PROGRAMS 1 0
	    set array(EXPERIMENTAL_PROGRAM) "BP3"
    ...
    } elseif { [string compare $premade_pipeline "SHELXCDE/ARPWARP"] == 0 } {
	set array(EXPERIMENTAL_PROGRAM) "PREP"
	update_toggleframe0 experiment_proc 0 $arrayname N_PROGRAMS 1 0
	set array(EXPERIMENTAL_PROGRAM) "SHELXC"
	update_toggleframe0 experiment_proc 0 $arrayname N_PROGRAMS 1 0
    ...
    } elseif { [string compare $premade_pipeline "CRUNCH2/BP3/SOLOMON/RESOLVEDM/MB"] == 0 } {
	set array(EXPERIMENTAL_PROGRAM) "PREP"
	update_toggleframe0 experiment_proc 0 $arrayname N_PROGRAMS 1 0
	...
	if { ( ( ([ string compare $start "Substructure detection"] == 0) ||
		 ([ string compare $start "Substructure phasing"]   == 0) ||
		 ([ string compare $start "Density modification"]   == 0) ) &&
	       !$done && [file exists [FindExecutable resolve_giant]] ) } {
	    set array(EXPERIMENTAL_PROGRAM) "SOLOMON"
	    update_toggleframe0 experiment_proc 0 $arrayname N_PROGRAMS 1 0
	    set array(EXPERIMENTAL_PROGRAM) "RESOLVEDM"
	...
	if { [file exists [FindExecutable resolve_giant]] && !$done} {
	    set array(EXPERIMENTAL_PROGRAM) "RESOLVEMB"
	    if { $array(DISPLAY_COOT) } {
		update_toggleframe0 experiment_proc 0 $arrayname N_PROGRAMS 1 0
	    } 
	    set done 1
    ...
    } elseif { [string compare $premade_pipeline "CRUNCH2/BP3/SOLOMON/BUCCANEER"] == 0 } {
	set array(EXPERIMENTAL_PROGRAM) "PREP"
	update_toggleframe0 experiment_proc 0 $arrayname N_PROGRAMS 1 0
	...
	if { [file exists [FindExecutable cbuccaneer]] && !$done} {
	    set array(EXPERIMENTAL_PROGRAM) "BUCCANEER"
	    if { $array(DISPLAY_COOT) } {
		update_toggleframe0 experiment_proc 0 $arrayname N_PROGRAMS 1 0
	    } 
	    set done 1
    ...
    } elseif { [string compare $premade_pipeline "CUSTOM"] == 0 } {
	set array(EXPERIMENTAL_PROGRAM) "PREP"
	update_toggleframe0 experiment_proc 0 $arrayname N_PROGRAMS 1 0
	...
	if { ( ( ([ string compare $start "Substructure detection"] == 0) ||
	    ...
	    if { [ string compare $array(REFINE_PROGS) "REFMAC" ] == 0 } {
		set array(EXPERIMENTAL_PROGRAM) "REFMAC"
 	    } else {
		set array(EXPERIMENTAL_PROGRAM) "BP3"
	    }
	...
	WarningMessage "The type ($premade_pipeline) was not found."
    }
...
proc rungetstuff { arrayname force } {
	...
	set input_pir "[GetFullFileName0 $arrayname SEQIN]"
	if {[ReadFile $input_pir sequence_text]} {
	    set data [split $sequence_text "\n"]
	    set formatted ""
	    set n 0
	    foreach line $data {
		incr n
		if { ( ([string match $line {>*} ] == 0) && ($n > 2) ) } {
	...
	set gcx_exec [file join $env(CRANK) bin gcx]
    } else {
	set gcx_exec [file join $env(CCP4) bin gcx] 
    }
    
    if { $nmonomers == 0 } {
	if {$input_pir != ""} {
	    set command "$gcx_exec HKLIN $input_mtz SEQIN $input_pir << \" NOUT \n$crystal_input\""
	} else {
	    set command "$gcx_exec HKLIN $input_mtz << \"$input_res\n NOUT \n$crystal_input\""
	}
    } else {
	if {$input_pir != "" } {
	    set command "$gcx_exec HKLIN $input_mtz SEQIN $input_pir << \"NMON $nmonomers\nNOUT \n$crystal_input\""
	} else {
	    set command "$gcx_exec HKLIN $input_mtz << \"$input_res\nNMON $nmonomers\nNOUT \n$crystal_input\""
	}
    }
	...
	if { ($array(SEQIN) != "") && ($array(DOCK_SEQUENCE) == 1) } {
	    ...
	    if { ![regexp {.*The number of residues determined from SEQIN is * *([0-9]*).*} $output full numres] } {
		puts "Residues from SEQIN could not be determined\n"
	    }    
	    set array(NRESIDUES) $numres
	}
...
#
proc crunch2_setup { typedefVar arrayname } {
    ...
    if { ! ([file exists [file join $env(CCP4) bin gcx] ] || 
	    [file exists [file join $env(CRANK) bin gcx] ]) } {
        WarningMessage "Can not find executible gcx - please ensure it is in your path"
        return 0
    }
    ...
    DefineMenu _refine_progs [list BP3 REFMAC ] 

    DefineMenu _hand_progs [list SOLOMON ] 

    if { ([file exists [FindExecutable resolve_giant]]) } {
	set resolve 1
	DefineMenu _dm_progs [list SOLOMON DM PIRATE RESOLVEDM ]
    } else {
	set resolve 0
 	DefineMenu _dm_progs [list SOLOMON DM PIRATE ] 
    }
	...
	DefineMenu _pipeline_step [list "Substructure detection" "Substructure phasing" "Density modification" "Model building" ] \
	    [list DETECT PHASE DM BUILD]

    } else {
	DefineMenu _pipeline_step [list "Substructure detection" "Substructure phasing" "Density modification" ] \
 	    [list DETECT PHASE DM]
	...
	if { $arpwarp } {
 	    if { $buc } {
		if { $resolve } {
		    DefineMenu _premade_pipeline [list CRUNCH2/BP3/SOLOMON/ARPWARP SHELXCDE/BP3/SOLOMON/ARPWARP SHELXCDE/ARPWARP CRUNCH2/BP3/SOLOMON/RESOLVEDM/MB CRUNCH2/BP3/SOLOMON/BUCCANEER CUSTOM ] 
		} else {
		    DefineMenu _premade_pipeline [list CRUNCH2/BP3/SOLOMON/ARPWARP SHELXCDE/BP3/SOLOMON/ARPWARP SHELXCDE/ARPWARP CRUNCH2/BP3/SOLOMON/BUCCANEER CUSTOM ] 
		}
	    } else {
		if { $resolve } {
		    DefineMenu _premade_pipeline [list CRUNCH2/BP3/SOLOMON/ARPWARP SHELXCDE/BP3/SOLOMON/ARPWARP SHELXCDE/ARPWARP CRUNCH2/BP3/SOLOMON/RESOLVEDM/MB CUSTOM ] 
		} else {
		    DefineMenu _premade_pipeline [list CRUNCH2/BP3/SOLOMON/ARPWARP SHELXCDE/BP3/SOLOMON/ARPWARP SHELXCDE/ARPWARP CUSTOM ] 
		}
	    } 
	} else {
	    if { $buc } {
		if { $resolve } {
		    DefineMenu _premade_pipeline [list CRUNCH2/BP3/SOLOMON/RESOLVEDM/MB CRUNCH2/BP3/SOLOMON/BUCCANEER CUSTOM ] 
		} else {
		    DefineMenu _premade_pipeline [list CRUNCH2/BP3/SOLOMON/BUCCANEER CUSTOM ] 
		}
	    } else {
 		if { $resolve } {
		    DefineMenu _premade_pipeline [list CRUNCH2/BP3/SOLOMON/RESOLVEDM/MB CUSTOM ] 
		} else {
		    DefineMenu _premade_pipeline [list CUSTOM ] 		
		}
	    }
	} 
    } else {
	if { $arpwarp } {
 	    if { $buc } {
		if { $resolve } {
		    DefineMenu _premade_pipeline [list CRUNCH2/BP3/SOLOMON/ARPWARP CRUNCH2/BP3/SOLOMON/RESOLVEDM/MB CRUNCH2/BP3/SOLOMON/PIRATE/BUCCANEER CUSTOM ] 
		} else {
		    DefineMenu _premade_pipeline [list CRUNCH2/BP3/SOLOMON/ARPWARP CRUNCH2/BP3/SOLOMON/BUCCANEER CUSTOM ] 
		}
 	    } else {
		if { $resolve } {
		    DefineMenu _premade_pipeline [list CRUNCH2/BP3/SOLOMON/ARPWARP CRUNCH2/BP3/SOLOMON/RESOLVEDM/MB CUSTOM ] 
		} else {
		    DefineMenu _premade_pipeline [list CRUNCH2/BP3/SOLOMON/ARPWARP CUSTOM ] 
		}
	    }
	} else {
 	    if { $buc } {
    ...
    DefineMenu _pipeline_start [list "Substructure detection" \
			       "Substructure phasing" ] [list DETECT PHASE]
...
#
proc crunch2_task_window { arrayname } {
    upvar #0 $arrayname array
    global env
    global param_list
    global programs_toggleframe_list
...
#    pack $logo -side left
	...
	-command "setup_experiment $arrayname" 
...
#    CreateLine line \
#	label "Pipeline : " \
#	message "Add a pre-built plan to the Crank pipeline" \
#	widget PREMADE_PIPELINE -width 40 \
#	-command "fix_scroll $arrayname ; SetSystemVar block_scroll_update 0 ; add_premade_pipeline $arrayname"

    OpenSubFrame frame -toggle_display PREMADE_PIPELINE open "CUSTOM"    
    ...
    OpenSubFrame frame -toggle_display PREMADE_END hide "PHASE"    
    ...
    OpenSubFrame frame -toggle_display PREMADE_END hide "DM"    
	...
	message "Clear all programs from the current pipeline" \
	label "  " \
	button "Clear All" \
	-command "clear_pipeline $arrayname" 
