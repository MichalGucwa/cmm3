...
#------------------------------------------------------------------------
  proc phaser_CCA_setup { typedefVar arrayname } {
#------------------------------------------------------------------------
  upvar #0 $typedefVar typedef
  upvar #0 $arrayname array

  DefineMenu _composition_option \
    [ list "solvent content of protein crystal" "components in asymmetric unit" ] [ list "SOLVENT" "COMPONENT"]
...
#------------------------------------------------------------------------
proc phaser_CCA_run { arrayname } {
#------------------------------------------------------------------------

  upvar #0 $arrayname array

# The output file array is set up in phaser_CCA.script
...
#------------------------------------------------------------------------
 proc phaser_CCA_component { arrayname counter_component } {
#------------------------------------------------------------------------
...
#------------------------------------------------------------------------
 proc phaser_CCA_set_test_space_group { arrayname } {
#------------------------------------------------------------------------
...
# update the list of spacegroups varlabel
  phaser_CCA_updatevarlabel $arrayname FILE_SPACEGROUP
  if { $laue_no <= 0 } {
...
#------------------------------------------------------------------------
 proc phaser_CCA_updatevarlabel { arrayname FILE_SPACEGROUP } {
#------------------------------------------------------------------------
...
#
#------------------------------------------------------------------------
  proc phaser_CCA_set_reso {arrayname} {
#------------------------------------------------------------------------
  upvar #0 $arrayname array
...
#------------------------------------------------------------------------
  proc phaser_CCA_task_window {arrayname} {
#------------------------------------------------------------------------
...
#   whether rotation, translation or refinement/rescoring searches
  ...
  CreateInputFileLine line \
      ...
      HKLIN_MAIN DIR_HKLIN_MAIN \
        ...
        -command "phaser_CCA_set_test_space_group $arrayname" \
        -command "phaser_CCA_set_reso $arrayname"
...
# The "Defie composition" folder
  ...
  CreateExtendingFrame N_COMPONENT phaser_CCA_component \
