...
#--------------------------------------------------------------------
proc scala_sym_setup { typedefname arrayname } { 
#--------------------------------------------------------------------
...
# If not defining multi runs then make sure everything set properly
  ...
  set array(HKLINX,1) $array(HKLIN)
  set array(DIR_HKLINX,1) $array(DIR_HKLIN)
...
#----------------------------------------------------------------------
proc scala_update_process_display_mode { arrayname } {
#----------------------------------------------------------------------  
  upvar #0 $arrayname array

  # can't have SD_CORRECT && !REFINE
  if { !$array(REFINE) } {
      set array(SD_CORRECT) 0
  }
...
#-----------------------------------------------------------------------
proc  scala_sym_task_window { arrayname } {
#-----------------------------------------------------------------------
  ...
  if { [CreateTaskWindow $arrayname  \
        "Scala - Pointless, Scala, Truncate" "Scala" \
        [ list \
...
#=PROTOCOL==============================================================
  ...
  #-------------------------------------------------------------------
  #  Option to skip scaling or SD refinement
  #-------------------------------------------------------------------
  OpenSubFrame frame -toggle_display CUSTOM_SCALA open 1
	...
	help "cycles" \
	widget REFINE -command "scala_update_process_display_mode $arrayname" \
	label "Refine scale factors" \
	message "Analyse discrepancies and correct SDs (SDCORRECTION)" \
	help sdcorrection \
	widget SD_CORRECT -command "scala_update_process_display_mode $arrayname"\
...
#------------------------------------------------------------------Define datasets
  ...
  OpenFolder 2 DATASET_DISPLAY_MODE hide NONE open
...
#---------------------------------------------------------scaling protocols
  ...
  OpenFolder 4 MULTI_RUNS open 1 hide
   ...
   OpenFolder 5 MULTI_RUNS open 0 hide
...
#------------------------------------------------------------------MTZ output
  ...
  OpenFolder 14 OUTPUT hide POLISH closed
...
###  OpenFolder 16 SHOW_TRUNCATE hide 0
  OpenFolder 16 closed
