...
proc rename_input_columns { step mtzin mtzout } {
    ...
    set sftools_script "$sftools_script set label col PHIDM \n $outphib\n set label col FOMDM \n $outfom \n"

    set phasingcoefficients 1

    if { [info exists XMLParse([join "crank soap run step=$step dm hlphasing_coefficients " __])] } {
	set phasingcoefficients $XMLParse([join "crank soap run step=$step dm hlphasing_coefficients" __])
    } 
	...
	set sftools_script "$sftools_script set label col HLADM \n $outhla\n set label col HLBDM \n $outhlb \n"
	set sftools_script "$sftools_script set label col HLCDM \n $outhlc\n set label col HLDDM \n $outhld \n "
    }
