...
#
#
# Wrapper for Mafft the multiple alignment program 
...
class Mafft:
   """ A class to call Mafft, the sequence multiple alignment program, to do 
       alignmnet of target and template sequences."""
 
   def __init__(self):

      if os.path.isfile(os.path.join(os.environ['CCP4'], 'libexec', 'mafft')):
         self.mafft_EXE=os.path.join(os.environ['CCP4'], 'libexec', 'mafft')
   ...
   def Mafft(self, input_fasta_file, output_fasta_file, logfile):
      """ A function to do a system call to Mafft."""
      ...
      command_line =  self.mafft_EXE + ' --quiet --maxiterate 1000 --nofft ' + self.input_fasta_file 
         
      if self.debug:
         sys.stdout.write("\n")
         sys.stdout.write("======================\n")
         sys.stdout.write("Mafft command line:\n")
         sys.stdout.write("======================\n")
      ...
      outputFile=open(self.output_fasta_file, "w")
...
if __name__ == '__main__':
   ...
   mafft=Mafft()
   mafft.Mafft('1vrd_all.fasta', 'out_1vrd.fasta')
