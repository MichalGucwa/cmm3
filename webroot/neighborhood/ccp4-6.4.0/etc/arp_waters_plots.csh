#!/bin/csh -f
set logf = $1 
set ncyc = `grep NCYC $logf | awk '{print $4}' | head -1`
set nrestr = 1 
@ ncyc++

if (`grep -c 'CGRAD' ${logf}` != 0 ) then
        @ ncyc = $ncyc + 1
        @ nrestr = $ncyc - 1
endif

if (`grep -c 'CDIR' ${logf}` != 0 ) then
        @ ncyc = $ncyc + 1
        @ nrestr = $ncyc - 1
endif

if (`grep -c 'CCP PROGRAM SUITE: PROTIN' ${logf}` != 0 ) then
	@ nrestr = $ncyc - 1 
endif		
...
if (`grep -c '_Free_correlation_coeff_Fo_to_Fc' ${logf} ` == 0 ) then
    cp t11.log t12.log
    cp t11.log t13.log
endif
